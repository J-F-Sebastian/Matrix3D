...
int main(int argc, char *argv[])
{
    ...
    alfad = -2.0*M_PI/(float)(points);
    betad = 0.5f*M_PI/(float)((circles + 1)/2);
    ...
    while (beta < M_PI/2)
    {
        ...
        while (countp++ < points)
        {
            x = 100.0f*cos(beta)*cos(alfa);
            y = 100.0*sin(beta);
            z = 100.0f*cos(beta)*sin(alfa);
            fprintf(out, "    {{%f, %f, %f, 1.0}}, //%d\n", x, y, z, counter++);
            alfa += alfad;
        }
        fprintf(out, "%s\n", "    /* Lower circle */");
    ...
    fprintf(out, "%s\n", "    /* Top */");
    fprintf(out, "    {{0.0, 100.0, 0.0, 1.0}}, //%d\n", counter++);
    fprintf(out, "%s\n", "    /* Bottom */");
    fprintf(out, "    {{0.0, -100.0, 0.0, 1.0}} //%d\n", counter++);
    fprintf(out, "%s\n", "};\n");
